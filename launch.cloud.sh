...
gpu_setup() {
    ...
    trap - INT
    $ssh_cmd -o StrictHostKeyChecking=no 'cd /home && (git clone https://github.com/tankgauravgt/huggingface-llm-course || echo "Repo may already exist, continuing...") && cd /home/huggingface-llm-course && bash ./setup.gpu.sh'
    local rc=$?
    trap '' INT
    if [[ $rc -ne 0 ]]; then
